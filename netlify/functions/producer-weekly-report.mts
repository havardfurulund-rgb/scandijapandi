// Scheduled function — runs every Monday at 08:00 Oslo time.
// Sends each producer a weekly report of their product performance.
...
export default async (_req: Request) => {
  ...
  // Get all producers with active products
  const producers = await db.sql`
    SELECT DISTINCT producer, producer_email
    FROM products
    WHERE active = TRUE AND producer_email IS NOT NULL
  `;
  ...
  for (const p of producers as any[]) {
    ...
    const products = await db.sql`
      ...
      WHERE producer_email = ${p.producer_email} AND active = TRUE
    `;

    // Get orders for their products in the last 7 days
    const orders = await db.sql`
      ...
      WHERE o.created_at >= ${fmt(periodStart)}
        ...
        AND o.producer_endpoint = ${p.producer_email}
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#ECE7DB;padding:32px 12px;">
    ...
    try {
      ...
      await db.sql`
        ...
        VALUES (${p.producer_email}, ${fmt(periodStart)}, ${fmt(periodEnd)}, ${(orders as any[]).length}, ${Math.round(totalRevenue * 100)})
      `;
      sent++;
    } catch (err) {
      console.error(`[producer-report] failed for ${p.producer_email}:`, err);
    }
  }

  return Response.json({ ok: true, sent, total: producers.length });
};

export const config: Config = {
  schedule: "0 7 * * 1",  // Every Monday at 07:00 UTC (09:00 Oslo)
};
